...
class ObjectTrackingPipeline:
    def __init__(
        ...
        self.detector = OWLv2Detector(owlv2_checkpoint, device=device)
        self.sam_wrapper = SAM2VideoWrapper(sam2_checkpoint, sam2_config, device=device)
        self.tracker = ObjectTracker(iou_threshold=0.5, embedding_threshold=0.8)
...
def main():
    ...
    parser.add_argument("--owlv2-checkpoint", required=True, help="Path to OWLv2 checkpoint")
