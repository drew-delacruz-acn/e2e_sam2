...
class ContrastiveTrainer:
    ...
    def initialize_representatives(self, embeddings: torch.Tensor, labels: torch.Tensor, 
        ...
        if init_method == 'class_means':
            ...
            with torch.no_grad():
                self.representatives.data.normal_(mean=0.0, std=1.0)
                
        elif init_method == 'bounded_random':
            # Random initialization within the bounds of actual embeddings
            with torch.no_grad():
                # Get min/max bounds from embeddings
                embed_min = embeddings.min(dim=0)[0]
                embed_max = embeddings.max(dim=0)[0]
                
                # Initialize uniformly within bounds
                for class_idx in range(num_classes):
                    self.representatives.data[class_idx] = torch.rand(embedding_dim, device=self.device) * (embed_max - embed_min) + embed_min
